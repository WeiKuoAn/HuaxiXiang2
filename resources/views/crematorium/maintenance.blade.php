                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="filter-form">
                            <form method="GET" action="{{ route('crematorium.maintenance') }}" class="row g-3"
                                id="filterForm">
                                <div class="col-md-2">
                                    <div class="filter-label">狀態</div>
                                    <select name="status" class="form-select" onchange="this.form.submit()">
                                        <option value="0" {{ $request->get('status') == '0' ? 'selected' : '' }}>未檢查
                                        </option>
                                        <option value="3" {{ $request->get('status') == '3' ? 'selected' : '' }}>送審
                                        </option>
                                        <option value="9" {{ $request->get('status') == '9' ? 'selected' : '' }}>已檢查
                                        </option>
                                        <option value="all" {{ $request->get('status') == 'all' ? 'selected' : '' }}>全部狀態</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="date" name="start_date" class="form-control"
                                        value="{{ $request->get('start_date') }}" onchange="this.form.submit()">
                                </div>

                                <div class="col-md-2">
                                    <div class="filter-label">結束日期</div>
                                    <input type="date" name="end_date" class="form-control"
                                        value="{{ $request->get('end_date') }}" onchange="this.form.submit()">
                                </div>
                                        ...
                                        @if (isset($staff))
                                            @foreach ($staff as $person)
                                                <option value="{{ $person->id }}"
                                                    {{ $request->get('inspector') == $person->id ? 'selected' : '' }}>
                                                    {{ $person->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select name="maintainer" class="form-select" onchange="this.form.submit()">
                                        ...
                                        @if (isset($staff))
                                            @foreach ($staff as $person)
                                                <option value="{{ $person->id }}"
                                                    {{ $request->get('maintainer') == $person->id ? 'selected' : '' }}>
                                                    {{ $person->name }}
                                                </option>
                                            @endforeach
                                        @endif
                            ...
                            <tbody>
                                @forelse($maintenance as $key => $record)
                                    <tr>
                                        ...
                                        <td>{{ $record->maintenance_date ? \Carbon\Carbon::parse($record->maintenance_date)->format('Y-m-d') : '-' }}
                                        </td>
                                        ...
                                        <td>
                                            @php
                                                $problemCount = $record->maintenanceDetails
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    @if ($record->status == 0)
                                                        <a class="dropdown-item"
                                                            href="{{ route('crematorium.editMaintenance', $record->id) }}"><i
                                                                class="mdi mdi-pencil me-2 font-18 text-muted vertical-middle"></i>編輯</a>
                                                    @endif
                                                    @if ($record->status == 9)
                                                        <a class="dropdown-item"
                                                            href="{{ route('crematorium.checkMaintenance', $record->id) }}"><i
                                                                class="mdi mdi-eye me-2 font-18 text-muted vertical-middle"></i>查看</a>
                                                    @endif
                                                    @if ($record->status == 3)
                                                        <a class="dropdown-item"
                                                            href="{{ route('crematorium.checkMaintenance', $record->id) }}"><i
                                                                class="mdi mdi-check me-2 font-18 text-muted vertical-middle"></i>
