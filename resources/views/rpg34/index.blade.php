@extends('layouts.vertical', ['page_title' => '每月骨灰罐/紀念品報表'])
...
@section('content')
    ...
    <div class="container-fluid">
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">每月骨灰罐/紀念品報表</li>
                        </ol>
                    </div>
                    <h4 class="page-title">每月骨灰罐/紀念品報表</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
        ...
        @if (isset($datas['products']) && count($datas['products']) > 0)
            <!-- 總計 -->
            <div class="row mb-4">
                <div class="col-12">
                    <div class="alert alert-info d-flex justify-content-between align-items-center mb-0">
                        <h3 class="mb-0">總銷售數量</h3>
                        <h2 class="mb-0"><strong>{{ number_format($datas['grand_total']) }}</strong> 件</h2>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card">
                            <div class="card-header category-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="category-count">{{ number_format($category['category_total']) }} 件</span>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="row g-3">
                                            ...
                                            <div class="col-md-6 col-lg-4">
                                                <div class="product-card rounded p-3 h-100">
                                                    <div class="d-flex justify-content-between align-items-start mb-3">
                                                        ...
                                                        <div class="product-quantity">{{ number_format($product['num']) }}
                                                        </div>
                                                    </div>

                                                    @if ($product['has_variants'] && isset($product['variants']))
                                                        @php
                                                            $active_variants = [];
                                                            ...
                                                            $has_multiple = count($active_variants) > 1;
                                                        @endphp

                                                        @if ($has_multiple)
                                                            <div class="variant-section">
                                                                <div class="variant-title">規格明細</div>
                                                                @foreach ($active_variants as $variant)
                                                                    <div
                                                                        class="variant-item d-flex justify-content-between">
                                                                        <span>{{ $variant['name'] }}</span>
                                                                        <strong
                                                                            class="text-primary">{{ number_format($variant['num']) }}</strong>
                                                                    </div>
                                                                @endforeach
                                                            </div>
                                                        @else
                                                            <div class="text-muted mt-2">
                                                                規格：{{ $active_variants[0]['name'] ?? '標準' }}
                                                            </div>
                                                        @endif
                                                    @endif
