...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <form action="{{ route('pay.create.data') }}" method="POST" id="your-form" enctype="multipart/form-data"
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                        ...
                                        <table id="cart" class="table cart-list">
                                            ...
                                            <tbody valign="center" align="center">
                                                @for ($i = 0; $i < 1; $i++)
                                                    <tr id="row-{{ $i }}">
                                                        <td>
                                                            <button class="mobile btn btn-primary del-row"
                                                                alt="{{ $i }}" type="button" name="button"
                                                                onclick="del_row(this)">刪除</button>
                                                        </td>
                                                        <td scope="row">
                                                            <input id="pay_date-{{ $i }}"
                                                                class="mobile form-control" type="date"
                                                                name="pay_data_date[]" value="{{ $create_today }}" required>
                                                        </td>
                                                        <td>
                                                            <select id="pay_id-{{ $i }}"
                                                                ...
                                                                required>
                                                                @foreach ($pays as $pay)
                                                                    <option value="{{ $pay->id }}" class="mobile">
                                                                        {{ $pay->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input id="pay_price-{{ $i }}"
                                                                ...
                                                                name="pay_price[]" value="" required>
                                                        </td>
                                                        <td>
                                                            <select id="pay_invoice_type-{{ $i }}"
                                                                alt="{{ $i }}" class="mobile form-select"
                                                                aria-label="Default select example"
                                                                name="pay_invoice_type[]" onchange="chgInvoice(this)"
                                                                required>
                                                        ...
                                                        <td>
                                                            <input id="pay_invoice-{{ $i }}"
                                                                class="invoice mobile form-control" type="text"
                                                                name="pay_invoice_number[]" value=""
                                                                placeholder="請輸入發票號碼">
                                                            <input list="vender_number_list_q"
                                                                class="vendor mobile form-control"
                                                                id="vendor-{{ $i }}" name="vender_id[]"
                                                                placeholder="請輸入統編號碼">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-12">
                                            <input id="add_row" class="btn btn-secondary" type="button"
                                                name="" value="新增筆數">
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="col-12">
                    <div class="text-center mb-3">
                        ...
                        <button type="submit" id="btn_submit"
                            class="btn w-sm btn-success waves-effect waves-light">新增</button>
        ...
        $(document).ready(function() {
            ...
            $("#add_row").click(function() {
                ...
                <tr id="row-${rowCount}">
                    ...
                    <td scope="row">
                        <input id="pay_date-${rowCount}" class="mobile form-control" type="date" name="pay_data_date[]" value="{{ $create_today }}" required>
                    </td>
                    <td>
                        <select id="pay_id-${rowCount}" class="mobile form-select" aria-label="Default select example" name="pay_id[]" required>
                            @foreach ($pays as $pay)
                    ...
                    <td>
                        <input style="display: none;" id="pay_invoice-${rowCount}" class="invoice form-control" type="text" name="pay_invoice_number[]" placeholder="請輸入發票號碼">
            ...
            $("#your-form").on('submit', function(e) {
                ...
                let pay_total = 0;
                let rowCount = $('#cart tbody tr').length;

                // 計算所有支出金額的總和
                for (var i = 0; i < rowCount; i++) {
                    let priceVal = $('#pay_price-' + i).val();
                    if (priceVal && $('#row-' + i).length > 0) {
