...
class TaskController extends Controller
{
    public function index(Request $request)
    {
        ...
        // 如果有 assigned_to 篩選
        if ($assigned_to) {
            $datas = $datas->whereHas('items', function($query) use ($assigned_to) {
                $query->where('user_id', $assigned_to);
            });
        }
    ...
    public function ajax_store(Request $request)
    {
        ...
        return response()->json([
            'task' => [
                ...
                'assignee_ids' => collect($taskItems)->pluck('user_id')->all(),
            ]
        ]);
    }
    ...
    {
        $request->validate(['id' => 'required|exists:task_items,id']);

        $item = TaskItem::findOrFail($request->id);
        
        // 檢查權限：只有被指派的用戶或「不指定」任務才能完成
        ...
        // 檢查是否所有 TaskItem 都完成了
        $task = $item->task;
        $totalItems = $task->items()->count();
        $completedItems = $task->items()->where('status', 1)->count();
        
        if ($totalItems > 0 && $completedItems == $totalItems) {
            // 如果所有 TaskItem 都完成，將 Task 標記為完成
            $task->status = 1;
    ...
    public function update($id, Request $request)
    {
        ...
        // 如果任務狀態改為已完成，將所有 TaskItem 也標記為已完成
        if ($request->status == 1) {
            $task->items()->update([
                'status' => 1,
                'completed_at' => now()
    ...
    public function destroy($id)
    {
        ...
        // 刪除相關的 TaskItem（會自動刪除，因為有 CASCADE 約束）
        // 但為了確保資料一致性，我們先刪除 TaskItem
        $task->items()->delete();
