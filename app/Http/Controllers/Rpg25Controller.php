...
class Rpg25Controller extends Controller
{
    public function rpg25(Request $request)
    {
        ...
        $datas = [];
        $sums = [];
        $proms = Prom::where('type','C')->where('status','up')->get();
        ...
        return view('rpg25.index')->with('datas',$datas)
    ...
    public function detail(Request $request , $month , $prom_id)
    {
        ...
        return view('rpg25.detail')->with('datas',$datas)->with('prom',$prom)->with('year',$search_year)->with('month',$month);
    }
}
