...
class LampController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!isset($check_renew)) {
            $datas = Lamp::whereIn('renew', [0, 1]);
        } else {
            $datas = Lamp::where('renew', $check_renew);
        }
    ...
    public function create()
    {
        ...
        $lamp_types = LampType::where('status', 'up')->get();
        return view('lamp.create')->with('lamp_types', $lamp_types)->with('customers', $customers);
    }
    ...
    public function show($id)
    {
        $customers = Customer::orderby('created_at', 'desc')->get();
        $lamp_types = LampType::where('status', 'up')->get();
        $data = Lamp::where('id', $id)->first();
        $sales = Sale::where('customer_id', $data->customer_id)->distinct('pet_name')->whereNotNull('pet_name')->get();
        return view('lamp.edit')->with('data', $data)->with('lamp_types', $lamp_types)->with('sales', $sales)->with('customers', $customers);
    }
    ...
    public function delete($id)
    {
        ...
        $data = Lamp::where('id', $id)->first();
        $sales = Sale::where('customer_id', $data->customer_id)->distinct('pet_name')->whereNotNull('pet_name')->get();
        return view('lamp.del')->with('data', $data)->with('lamp_types', $lamp_types)->with('sales', $sales)->with('customers', $customers);
    }

    public function destroy(Request $request, $id)
    {
        Lamp::where('id', $id)->delete();
        return redirect()->route('lamps');
    }
    ...
    public function export(Request $request)
    {
        ...
        if (!isset($colse) || $colse == '1') {
            $fileName = '未結案-平安燈管理明細' . date("Y-m-d") . '.csv';
        } else {
            $fileName = '已結案-平安燈管理明細' . date("Y-m-d") . '.csv';
        }
        ...
        $header = array('平安燈起始日期', $start_date_start . '~', $start_date_end, '平安燈結束日期', $end_date_start . '~', $end_date_end);
        $columns = array('編號', '平安燈類別', '顧客名稱', '電話', '寶貝名稱', '目前簽約年份', '開始日期', '結束日期', '金額', '續約');
